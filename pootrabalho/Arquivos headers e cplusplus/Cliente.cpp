...
void Cliente::salvarArquivoCliente()
{
	ofstream file;
	...
	for (int i = 0; i < getQtdeApoliceCliente(); i++)
	{
		...
		file << "Apólice #" << i + 1 << "\n";
		file << "Número: " << apolice[i]->getNumApolice() << "\n";
		...
		apolice[i]->salvarArquivoApolice();
		
		if (apolice[i]->getQtdeOrcamento() == 0)
		{
			file.open("data.txt", ios::app);
			file << "Não possui orçamentos vinculados" << "\n\n";
		}
		else
		{
			apolice[i]->salvarArquivoOrcamentoApolice();
		}
		...
		if (apolice[i]->getQtdeSinistro() == 0)
		{
			file << "Não possui sinistros vinculados" << "\n\n";	
		}
		else
		{
			file.close();
			apolice[i]->salvarArquivoSinistroApolice();
		}
